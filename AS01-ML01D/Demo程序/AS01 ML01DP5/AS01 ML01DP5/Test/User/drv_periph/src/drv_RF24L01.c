...
void NRF24L01_Set_Speed( nRf24l01SpeedType Speed )
{
	...
	NRF24L01_Write_Reg( RF_SETUP, btmp ); //写入寄存器
}
...
void RF24L01_Set_Mode( nRf24l01ModeType Mode )
{
    ...
    if( Mode == MODE_TX ) //发送模式      
	{
		controlreg &= ~( 1<< PRIM_RX );
	}
    else 
	{
		if( Mode == MODE_RX ) //接收模式 
		{ 
			controlreg |= ( 1<< PRIM_RX ); 
		}
	}

    NRF24L01_Write_Reg( CONFIG, controlreg );//写入模式
}
...
uint8_t NRF24L01_TxPacket( uint8_t *txbuf, uint8_t Length )
{
	...
	RF24L01_SET_CS_LOW( );		//片选
	drv_spi_read_write_byte( FLUSH_TX );//清空TX缓冲区
	RF24L01_SET_CS_HIGH( );		//取消片选
...
uint8_t NRF24L01_RxPacket( uint8_t *rxbuf )
{
	...
	RF24L01_SET_CS_LOW( );		//片选
	drv_spi_read_write_byte( FLUSH_RX );	//清空RX缓存区
	RF24L01_SET_CS_HIGH( );		//取消片选
...
//	RCC_APB2PeriphClockCmd( RF24L01_CE_GPIO_CLK | RF24L01_IRQ_GPIO_CLK, ENABLE );	//?? CE IRQ????  CS SCK MISO MOSI?SPI?????
	...
	RF24L01_GpioInitStructer.Speed = GPIO_SPEED_FREQ_MEDIUM;
...
void RF24L01_Init( void )
{
    ...
    NRF24L01_Write_Reg( FEATRUE, 0x07 );		//设置特征寄存器，使能动态负载长度，使能ACK负载，使能命令W_TX_PAYLOAD_NOACK
    ...
    NRF24L01_Write_Reg( RF_SETUP, 0x26 );					//设置通信速度为1M
